...
impl fmt::Debug for Meta {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let mut res = vec![];
        if self.contains(Meta::FROZEN_FORM) {
            res.push("F")
        }
        if self.contains(Meta::SIMPLE_FORM) {
            res.push("S")
        }
        if self.contains(Meta::EXPAND_FORM) {
            res.push("E")
        }
        write!(f, "{}", res.join("|"))
    }
...
impl Expr {
    ...
    pub fn real(r: Real) -> Self {
        ...
        if r.is_negative() { Expr::minus(e) } else { e }
    }

    #[inline]
    pub fn pow(mut self, exp: Expr) -> Expr {
        self.pow_with(exp, noctua_global_config().default_eval_strategy);
        self
    }
    pub fn sin(self) -> Expr {
        let meta = self.meta;
        Expr {
            typ: ExprTyp::TrigFn(TrigFnTyp::Sin, Atom::expr(self)),
            meta,
        }
    }
    pub fn cos(self) -> Expr {
        let meta = self.meta;
        Expr {
            typ: ExprTyp::TrigFn(TrigFnTyp::Cos, Atom::expr(self)),
            meta,
        }
    }
    ...
    #[log_fn]
    pub fn add_with(&mut self, mut rhs: Expr, strat: EvalStrategy) -> &mut Expr {
        if matches!(strat.add, AddStrategy::Frozen) {
            *self = Expr {
    ...
    #[log_fn]
    pub fn mul_with(&mut self, mut rhs: Expr, strat: EvalStrategy) -> &mut Expr {
        if matches!(strat.mul, MulStrategy::Frozen) {
            *self = Expr {
            ...
            MulStrategy::Simple => match (&mut self.typ, &mut rhs.typ) {
                ...
                _ => {
                    *self = Expr {
                        typ: ExprTyp::Prod(
                            [
                                Atom::expr(self.remove_expr()),
                                Atom::expr(rhs.remove_expr()),
                            ]
                            .into(),
                        ),
            ...
            MulStrategy::Base => {
                if rhs.is_prod() {
                    for oprnd in rhs.remove_nary_operands().unwrap() {
                        self.mul_with(Expr::atom(oprnd), strat);
                    }
                    ...
                    expon_expr.add_with(Expr::atom(r_exp), strat);
                    exp.cleanup_indirection();
                } else {
            ...
            MulStrategy::Expand => match (self.typ(), rhs.typ()) {
                (ExprTyp::Sum(_), _) => {
                    ...
                    for term in self.remove_nary_operands().unwrap() {
                        ...
                        prod.mul_with(rhs.clone(), strat);
                        sum.add_with(prod, strat);
                    }
                ...
                (_, ExprTyp::Sum(_)) => {
                    ...
                    for term in rhs.remove_nary_operands().unwrap() {
                        ...
                        prod.mul_with(Expr::atom(term), strat);
                        sum.add_with(prod, strat);
                    }
                ...
                (_, _) => {
                    self.mul_with(rhs, strat.with_mul(MulStrategy::Base));
                }
            },
    ...
    #[log_fn]
    pub fn pow_with(&mut self, mut expon: Expr, strat: EvalStrategy) -> &mut Expr {
        if matches!(strat.pow, PowStrategy::Frozen) {
            *self = Expr {
        ...
        match (self.typ(), expon.typ()) {
            ...
            (_, &ONE) => return self,
            _ => {
                let (br, er) = (self.as_real(), expon.as_real());

                if let (Some(base), Some(expon)) = (br, er) {
                    ...
                    *self = res;
                    return self;
                } else if let (ExprTyp::Pow([_, _]), Some(_)) = (&self.typ, &er) {
                    let [b_base, b_expon] = self
                        .remove_binary_operands()
                        .expect("self is a power expression");
                    let mut prod = Expr::atom(b_expon);
                    prod.mul_with(expon, strat);
                    *self = Expr {
                        typ: ExprTyp::Pow([b_base, Atom::expr(prod)].into()),
                        meta: Meta::SIMPLE_FORM,
                    };
                    return self;
                }
            ...
            PowStrategy::Expand => {
                match (self.typ(), expon.typ()) {
                    (ExprTyp::Pow(_), _) => {
                        ...
                        e.mul_with(expon, strat);
                        let mut pow = Expr::atom(b);
                        pow.pow_with(e, strat);
                        *self = pow;
                    }
                    (ExprTyp::Prod(_), _) => {
                        ...
                        for op in self.remove_nary_operands().unwrap() {
                            ...
                            pow.pow_with(expon.clone(), strat);
                            prod.mul_with(pow, strat);
                        }
                    }
                    (ExprTyp::Sum(_), ExprTyp::Atom(Atom::U32(n))) if *n > 1 => {
                        ...
                        for k in 0..=n {
                            if k == 0 {
                                ...
                                a.pow_with(expon.clone(), strat);
                                sum.add_with(a, strat);
                            } else if k == n {
                                // 1 * rest^n
                                let mut b = rest.clone();
                                b.pow_with(expon.clone(), strat);
                                sum.add_with(b, strat);
                            } else {
                                ...
                                a.pow_with(Expr::u32(k), strat);
                                b.pow_with(Expr::u32(n - k), strat);

                                a.mul_with(Expr::u32(c), strat).mul_with(b, strat);
                                sum.add_with(a, strat);
                            }
                        }
    ...
    pub fn expand_root_mut(&mut self) -> &mut Expr {
        ...
        match &mut self.cleanup_mut().typ {
            ExprTyp::TrigFn(_, _) | ExprTyp::Atom(_) | ExprTyp::Sum(_) => (),
            ...
            ExprTyp::Prod(_) => {
                let mut prod = Expr::u32(1);
                for op in self.remove_nary_operands().unwrap() {
                    prod.mul_with(Expr::atom(op), EvalStrategy::expand());
                }
                *self = prod;
            }
            ...
            ExprTyp::Pow(_) => {
                ...
                pow.pow_with(Expr::atom(expon), EvalStrategy::expand());
                *self = pow;
            }
        }
    ...
    pub fn inline_trivial_compound(&mut self) -> &mut Expr {
        match &mut self.typ {
            ...
            ExprTyp::Minus(Atom::Expr(e)) if e.is_minus() => match &mut Rc::make_mut(e).typ {
                ExprTyp::Minus(a) => {
    ...
    pub fn operands(&self) -> &[Atom] {
        match self.typ() {
            ExprTyp::TrigFn(_, atom) | ExprTyp::Atom(atom) | ExprTyp::Minus(atom) => {
    ...
    pub fn base_expon_ref(&self) -> (View<'_>, &Atom) {
        match self.typ() {
            ExprTyp::Pow([base, expon]) => (base.view(), expon),
    ...
    #[inline]
    pub fn is_trig_and(&self, f: impl FnOnce(&TrigFnTyp, &Atom) -> bool) -> bool {
        self.dbg_assert_valid();
        match &self.typ {
            ExprTyp::TrigFn(typ, a) => f(typ, a),
            _ => false,
        }
    }
    #[inline]
    ...
    pub fn eq_typ(&self, other: &Expr) -> bool {
        std::mem::discriminant(&self.typ) == std::mem::discriminant(&other.typ)
            && self.is_trig_and(|typ1, _| {
                other.is_trig_and(|typ2, _| {
                    std::mem::discriminant(typ1) == std::mem::discriminant(typ2)
                })
            })
    }
    ...
    pub fn simplified_ordering(&self, other: &Expr) -> cmp::Ordering {
        ...
        match (lhs.typ(), rhs.typ()) {
            ...
            (ExprTyp::TrigFn(typ1, _), ExprTyp::TrigFn(typ2, _)) => typ1.name().cmp(typ2.name()),
            (ExprTyp::TrigFn(_, _), _) => cmp_views(ops_view(lhs), expr_view(rhs)),
            (_, ExprTyp::TrigFn(_, _)) => cmp_views(expr_view(lhs), ops_view(rhs)),
    ...
    fn dbg_assert_valid(&self) {
        match &self.typ {
            ...
            ExprTyp::TrigFn(_, _) | ExprTyp::Minus(_) | ExprTyp::Pow(_) => (),
        }
        // self.
    }
}
...
impl ops::Add for Expr {
    ...
    fn add(mut self, rhs: Self) -> Self::Output {
        self.add_with(rhs, noctua_global_config().default_eval_strategy);
        self
    }
}
...
impl ops::Mul for Expr {
    ...
    fn mul(mut self, rhs: Self) -> Self::Output {
        self.mul_with(rhs, noctua_global_config().default_eval_strategy);
        self
    }
}
...
impl fmt::Debug for Expr {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self.typ() {
            ...
            ExprTyp::TrigFn(typ, atom) => write!(f, "{}({atom:?})", typ.name()),
        }?;
        write!(f, " ({:?})", self.meta)
    }
}
